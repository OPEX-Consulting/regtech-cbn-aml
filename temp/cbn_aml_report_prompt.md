...
You must return ONLY this JSON object. Every field is required. String fields must never be empty — use contextually appropriate content. Arrays must have the correct number of elements as specified.
...
```json
{
  "meta": {
    "inst_name": "string",
    "inst_type": "string",
    "inst_type_full": "string — full descriptive name e.g. 'International Money Transfer Operator (IMTO)'",
    "contact_name": "string",
    "contact_email": "string",
    "contact_role": "string",
    "report_date": "string — always 'April 2026'",
    "circular_ref": "string — always 'BSD/DIR/PUB/LAB/019/002'",
    "roadmap_deadline": "string — always '10 June 2026'",
    "compliance_deadline": "string — 'September 2027' for DMBs, 'March 2028' for all others",
    "compliance_deadline_basis": "string — e.g. '18 months from issuance' or '24 months from issuance'"
  },

  "overall_rating": {
    "rating": "string — one of: CRITICAL | HIGH | MEDIUM | LOW",
    "rating_label": "string — e.g. 'Critical compliance risk'",
    "summary_paragraph": "string — 3–5 sentence institution-specific narrative explaining the overall rating. Must reference: the institution type, aml_status, number of Critical Gap standards, governance score, and any elevated risk factors. Must NOT be generic. Must reference the CBN's explicit statement that AML Solutions without KYC linkage are non-compliant.",
    "sector_context_note": "string — 1–2 sentence note on why this institution type faces elevated or standard inherent risk under the Baseline Standards"
  },

  "scorecard": {
    "aml_system_status_label": "string — e.g. 'None in place' or 'Partial — 3 of 8 functions covered'",
    "aml_system_status_rating": "string — one of: Compliant | Gap Identified | Critical Gap",
    "standards_compliant_count": "integer 0–12",
    "standards_compliant_rating": "string — one of: Compliant | Gap Identified | Critical Gap",
    "standards_critical_gap_count": "integer 0–12",
    "standards_critical_gap_rating": "string — one of: Compliant | Gap Identified | Critical Gap | High",
    "standards_gap_identified_count": "integer 0–12",
    "governance_score_label": "string — e.g. '3 of 10 (30%)'",
    "governance_score_rating": "string — one of: Compliant | Gap Identified | Critical Gap | Elevated",
    "internal_audit_label": "string — e.g. 'Not covered' or 'Annually'",
    "internal_audit_rating": "string — one of: Compliant | Gap Identified | Critical Gap",
    "risk_factors_label": "string — brief description of key risk factors, e.g. 'Material fraud exposure, Cross-border FX' or 'None identified'",
    "risk_factors_rating": "string — one of: Elevated | Standard | Critical",
    "regulatory_context_box": "string — 2–3 sentences quoting the CBN's key compliance statement and explaining the examination evidence requirement and roadmap submission obligation"
  },

  "gap_analysis_intro": "string — 2–3 sentence introduction to the gap analysis section, naming the institution and the 12 standards being assessed",

  "standards": [
    {
      "section": "string — e.g. '§5.1'",
      "title": "string — e.g. 'AML Solution (Functional Footprint)'",
      "status": "string — one of: Compliant | Gap Identified | Critical Gap",
      "finding": "string — 3–6 sentences. Must be specific to this institution. Must explain what is missing or non-compliant, why it matters for this specific institution type and risk profile, and reference the specific CBN requirement being violated. Must NOT be copied verbatim from the standard — it is a finding about the institution.",
      "required_action": "string — 1–3 sentences stating the specific remediation action required. Should reference RegTech365 products where directly applicable."
    }
    // Repeat for all 12 standards: §5.1 through §5.12
  ],

  "governance_assessment": {
    "intro": "string — 1–2 sentence introduction to the governance assessment table",
    "items": [
      {
        "control": "string — governance control name",
        "status": "string — 'In place' | 'Not confirmed' | 'Not in place'"
      }
      // Exactly 10 items, in this order:
    ...
    "overall_score_label": "string — e.g. '3 of 10 controls confirmed (30%) — Weak'",
    "overall_score_rating": "string — one of: Critical | Weak | Partial | Adequate | Strong"
  },

  "priority_actions": [
    {
      "number": "integer 1–5",
      "title": "string — short action title e.g. 'Complete the CBN Implementation Roadmap Template'",
      "deadline_label": "string — e.g. 'Deadline: 10 June 2026' or 'Recommended completion: May 2026'",
      "body": "string — 3–5 sentences. Must be specific to this institution. Must explain what is required, why it is urgent for this institution, what happens if it is not done, and (where applicable) how OPEX Consulting or RegTech365 can help."
    }
    // Exactly 5 items
  ],

  "roadmap": {
    "intro": "string — 2–3 sentences introducing the recommended implementation roadmap, stating it is structured to achieve full compliance by the compliance deadline",
    "phases": [
      {
        "phase_number": "integer 1–4",
        "title": "string — e.g. 'Phase 1 — Foundation & Submission'",
        "timeline": "string — e.g. 'Now — June 2026'",
        "objectives": "string — 1–2 sentences describing the phase objective",
        "key_deliverables": "string — comma-separated list of 4–6 specific deliverables tailored to this institution",
        "standards_addressed": "string — comma-separated list of standards e.g. '§5.1, §5.2, Section 6'"
      }
      // Exactly 4 phases
    ]
  },
  ...
  "support_section": {
    "intro_paragraph": "string — 2–3 sentences introducing OPEX Consulting and RegTech365's integrated advisory and RegTech offering for Nigerian financial institutions",
    "advisory_intro": "string — 2–3 sentences describing OPEX Consulting's immediate roadmap submission support, tailored to this institution's June 2026 deadline",
    "products": [
      {
        "name": "string — product name e.g. 'RegPort'",
        "function": "string — brief function description",
        "standards_addressed": "string — standards this product addresses",
        "relevance_to_client": "string — 2–3 sentences explaining why this product is specifically relevant to THIS institution's gap profile and risk factors"
      }
      // Exactly 4 products: RegPort, RegGuard, RegComply, RegLearn
    ],
    "advisory_services": [
      "string — one advisory service line item"
      // Exactly 6 items drawn from: CBN Roadmap Template completion, AML/CFT/CPF Policy drafting, MLRO advisory, Vendor due diligence and platform selection, Internal audit co-sourcing, ISO 42001 AI governance alignment, Evidence pack preparation — select the 6 most relevant to this institution
    ],
    "next_steps_box": "string — 3–4 sentences inviting the institution to engage OPEX Consulting / RegTech365, mentioning the complimentary 60-minute advisory session, and providing the contact email compliance@opexconsulting.ng"
  },

  "disclaimer": "string — standard disclaimer text: 'This gap assessment report has been prepared by OPEX Consulting / RegTech365 based on information provided by the institution through the self-assessment questionnaire. It is intended for internal compliance planning purposes only and does not constitute legal advice. Institutions should engage qualified compliance and legal counsel for formal regulatory submissions. Findings reflect the position as at April 2026 and should be reviewed as the institution's circumstances change.'"
}
```
...
Use this exact structure when calling the API. Replace `{{INPUT_JSON}}` with the serialised assessment data.

```
You are generating a CBN AML gap assessment report. Below is the institution's self-assessment data. Apply all scoring logic, regulatory context, and output schema from your instructions to produce the report JSON.
...
Before the model finalises its response, it must internally verify:

- [ ] `meta.compliance_deadline` correctly reflects the institution type (DMB = September 2027, all others = March 2028)
- [ ] All 12 standards are present in `standards` array, in order §5.1–§5.12
- [ ] No standard finding is generic — each references the specific institution
- [ ] `overall_rating.rating` is consistent with the scorecard counts
- [ ] `scorecard.standards_compliant_count + standards_critical_gap_count + standards_gap_identified_count` = 12
- [ ] `governance_assessment.items` has exactly 10 items
- [ ] `priority_actions` has exactly 5 items
- [ ] `roadmap.phases` has exactly 4 phases
- [ ] `support_section.products` has exactly 4 products (RegPort, RegGuard, RegComply, RegLearn)
- [ ] `support_section.advisory_services` has exactly 6 items
- [ ] IMTO-specific references included where `inst_type` = IMTO (goAML, NFIU, cross-border ML/TF risk)
- [ ] All elevated risk factors referenced in relevant standard findings
- [ ] Output is valid JSON with no trailing commas or undefined values
